...
function RentPropertyForm() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.location || !formData.rent || !formData.bedroom || !formData.bathroom || !formData.area) {
  ...
  const handleDelete = (index) => {
    const updatedList = rentalList.filter((_, i) => i !== index);
    setRentalList(updatedList);
  };
  ...
  return (
    <div className="container mt-5 mb-5 p-4 border rounded shadow bg-white">
      ...
      <h2 className="mb-4 text-center">🏘️ Sell Out Your {propertyType}</h2>
